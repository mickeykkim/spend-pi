...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/genomicsengland/opensource/templates/spend-pi
...
.build_${PYTHON_VERSION}: Dockerfile spend_pi clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	mkdir -p $(CURRENT_LOCATION)/bin && \
	docker run \
			--platform linux/amd64 \
			--volume $(CURRENT_LOCATION)/bin/:/spend_pi/bin/ \
			--volume $(CURRENT_LOCATION):/spend_pi:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/spend_pi/bin/coverage.xml tests/
